    ...
    while True:
        ...
        for index, (cv_sink, output_stream, img) in enumerate(input_output[1:2]):
            ...
            if found_contour != None:
                ...
                #Network tables for sending data
            ...
            #Every 5 frames Average the center point
            if len(Recent_Points) >= RUNNING_AVERAGE_NUM:
                num_of_points = len(Recent_Points)
                average_point = reduce(sum_of_points, Recent_Points)

                #Clear for more data in Recent Points
                Recent_Points.clear()

                #If the there is points in Average point then begin to Average
                if average_point != None:
                    ...
                    #Draw a cross arrow on the Average
